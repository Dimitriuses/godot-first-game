...
The sixty-six spritesheets that make up the six dice were **rendered for this repository in
...
Kenney above. The `D6 Dotted`, `D20`, `D4`, `D6 Numbered`, `D8` and `D10` objects are used; the
same pack also holds a percentile D10 and a D12, under the same terms — which is what makes
